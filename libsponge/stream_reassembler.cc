...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
     ...
     *  存在两种情况：
     *  1. 新的序列刚好可以被传输，即 index <= _next_assembled_idx < index + data.size()，此时注意到序号 _next_assembled_idx 前的字节都已经传输了，因此新的序列在 _next_assembled_idx 之前的字节需要被截断
     *  2. 新的序列的开头部分与 map 中某个序列重叠，即 entry.key <= index && index < entry.key + entry.value.size()，此时需要把新的序列中的重叠部分去除，确保 map 中存储的所有序列都是不重叠的
     */

    /** 
     *  通过 upper_bound 找到 key 大于 index 的最小的 iter 存在以下情况
     *  1. iter == map.end() 所有的序列的 key 都小于等于 index，此时实际上不会发生截断
     *  2. iter == map.begin() 所有的序列的 key 都大于 index，此时看 index 是否满足情况 1(可能不满足)，如果满足情况 1 则截断前半部分
    ...
    // 截断操作，注意到 new_idx 可能大于 index + data.size()，对应的情况是情况 2 中 key <= index 最近的序列完全把新的序列包含
    // 即存在 entry.key <= index && entry.key + entry.value.size() > index + data.size()
     ...
     *  1. 该序列被新序列覆盖，即 new_idx <= entry.key && new_idx + data_size >= entry.key + entry.value.size()，此时将这个 entry 从 map 中删除并继续遍历
     *  2. 这个序列和新序列部分重叠，即 new_idx <= entry.key && new_idx + data_size < entry.key + entry.value.size()，此时在新序列中删除重叠部分，更新 data_size，结束遍历
     *  3. 没有重叠部分，结束遍历
     */
     ...
     *  按照 key 的大小遍历 map
     *  1. 如果 key == _next_assembled_idx，发送序列，如果全部发送成功继续遍历，如果部分发送成功，删除旧序列，插入未发送成功的那部分，结束遍历
