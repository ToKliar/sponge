...
void TCPSender::fill_window() {
    ...
    size_t curr_windows_size = _window_size ? _window_size : 1;
    ...
    while (curr_windows_size > _outgoing_size) {
        ...
        const size_t payload_size =
            min(TCPConfig::MAX_PAYLOAD_SIZE, curr_windows_size - _outgoing_size - segment.header().syn);
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    for (auto iter = _outgoing_map.begin(); iter != _outgoing_map.end();) {
        ...
        if (iter->first + seg.length_in_sequence_space() <= abs_ackno) {
            _outgoing_size -= seg.length_in_sequence_space();
...
void TCPSender::tick(const size_t ms_since_last_tick) {
    ...
    if (iter != _outgoing_map.end() && _time_pass >= _time_out) {
        if (_window_size > 0) {
            _time_out *= 2;
        }
        _time_pass = 0;
